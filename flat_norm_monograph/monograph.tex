...
What we would like to do is to find a characterization of these concepts that generalizes to a manifold - a structure that locally looks like $\mathbb{R}^d$ for some $d$, but may globally be very different (you may think of these as simply being reasonable shapes or surfaces). Some manifolds (like the unit sphere) can sit in $\mathbb{R}^n$ for some $n$ in a natural way, and so we may think of them as $d$-dimensional objects in $\mathbb{R}^n$. In fact, due to famous embedding theorems by Nash, we know we can always find a large enough $n$ to embed our manifolds in. However, some manifolds (such as the projective plane) would be tortured to think about as embedded, as they don't sit in $\mathbb{R}^n$ in a very natural way. Hence we will try to develop a more intrinsic notion of these concepts.

Think about the situation then whereby we are trying to develop a notion of a tangent space along with vectors to the suface of the Earth without referring to anything beyond its surface. If you like, for all intents and purposes we consider ourselves residents of Flatland as pictured by Edwin Abbott Abbott. Considering this problem for some time, we might consider one of our rather limited tools might be to draw smooth curves through a point $p$ on the Earth's surface by grabbing some chalk. It turns out that this is enough, if we are very clever in how we measure the velocity of the curve through the point at $p$. Which, as we will see in a second, needs to be written down explicitly.

Consider a curve $\gamma: \mathbb{R}\to M$ thought of as being traced out on the surface of the Earth $M$. Consider for a real life example, that we have placed little flags on the Earth and we enumerate them in the order we will be tracing our path in. (We go from flag 1, to flag 2, to flag 3, etc.) this defines the curve $\gamma$ as a function from $\mathbb{R}$ into points on the Earth $M$ (of course, we could not practically place as many flags as there are real numbers, but imagine we interpolate). Suppose that said curve $\gamma$ goes through the point $p$ of interest at flag $\lambda_0$ so that $\gamma(\lambda_0) = p \in M$. 

Then we define the velocity of the curve $\gamma$ at the point $p$ denoted $V_{\gamma,p}$ as a linear function $V_{\gamma, p}:C^\infty(M) \to \mathbb{R}$ where $C^\infty(M)$ is the set of all smooth functions $\{f: M \to \mathbb{R}, f \text{ is smooth}\}$. It might be strange to think of a velocity as a function, but bear with us. We define the action of the velocity on such a smooth curve $f$ by $V_{\gamma,p}(f) = (f \circ \gamma)'(\lambda_0) \in \mathbb{R}$. Here, $f$ should be regarded as a sort of stopwatch, as a function from $M \to \mathbb{R}$ it takes the point we're at and maps it to the time we reached it. Hence if we run along our flags and time ourselves as we do it, we can reasonably get a velocity through the point $p$ by differentiation. While it might seem strange to differentiate a path on the Earth and the mere thought might be leaving you wondering how you might do that, note that the composition of flags and the stopwatch means that $f \circ \gamma : \mathbb{R} \to \mathbb{R}$ is a function from $\mathbb{R} \to \mathbb{R}$, any undergraduate calculus student could take the derivative for us (so long as it is differentiable in the first place). Thus, we have put into mathematical formulation what most children before puberty. To determine ones speed as we run about the Earth, we mark landmarks where we intend to go and have a friend read the time as we reach them.
